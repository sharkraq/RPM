...
#Remove the scan engine from the r7console.
#This should only done on existing scanners, not to new scanners.
if test -s "/scanner/etc/scanner-id"; then
SID=`cat /scanner/etc/scanner-id`
curl -X DELETE -H "Content-type:application/x-www-form-urlencoded" -H "Accept:application/json" -k https://r7console.vuma.silversky.com:8443/report/scanengine/$SID
fi
...
for F in /etc/issue /etc/issue.net /etc/system-release; do
  sed -i "s/release.*/release %{iso_version}/" $F
done
sed -i s/version:string.*$// /etc/scanner.conf
echo "version:string				= %{iso_version}" >>/etc/scanner.conf
/sbin/service  nexposeengine.rc start
fi
